 ...
 **/
	int bestRotation(vector<int>& nums) {
		...
		vector<int> indexScore;//存储第K次调换的得分
		...
		//查询最大得分  最小索引
		for (int s=0;s<indexScore.size();s++)
		{
			if (indexScore[s]==maxScore)
			{
				index= s;
				break;
			}
